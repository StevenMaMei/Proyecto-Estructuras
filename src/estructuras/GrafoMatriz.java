		...
		for (int i = 0; i < maxNodos; i++) {
			for (int j = i + 1; j < maxNodos; j++) {
				if (matrizAdyacencia[i][j] != Double.MAX_VALUE) {
					NodoPesoAdyacente<NodoMatriz<E>> arista = new NodoPesoAdyacente<NodoMatriz<E>>(nodos.get(i),
							matrizAdyacencia[i][j], nodos.get(j));
					aristas.add(arista);
				}
			}
		}
			...
			if (!nuev.isRevisado()) {
				...
				for (int i = 0; i < totalNodos; i++) {
					...
					if (i != ind3 && matrizAdyacencia[ind3][i] != Double.MAX_VALUE && !nuevo.isRevisado()) {
						NodoPesoAdyacente<NodoMatriz<E>> p = new NodoPesoAdyacente<>(nuevo, matrizAdyacencia[ind3][i],
								nuev);
						colaPrior.add(p);
					}
				}
